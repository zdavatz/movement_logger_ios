...
- **Merge tab** — stitch several phone clips of one session into a single film, in chronological order, **never trimming a clip** ("never cut a video"). The film **opens on the first clip's first frame held ~3 s with the gradient "MovementLogger" title floating semi-transparently over it**, then plays each clip in full — each preceded by a black date/time title card and followed by a 3 s freeze of its last frame fading out — and **closes with an animated "pumping" foil-logo outro: the foil icon rocking about its wings (cadence + amplitudes mapped from real foil-pumping footage) over a sky→sea gradient, fading in from the last clip's fade-to-black and back out to black** to end the film. Only **portrait** clips are mergeable; landscape clips are held in a separate red "Not merged" section so a mixed selection can't blow the render canvas up to a square. Optionally overlays the same Replay sensor panels under each clip. Exports an H.264 `.mov` to `Documents/combined_<basename>.mov` (and Photos), stamped with the earliest clip's capture date. Every overlay card — intro, title cards, freeze frames, logo outro — is rendered as a short **media segment** rather than a CoreAnimation layer, so a plain merge carries an empty layer tree and skips the offline animation tool entirely; that's what lets long, many-clip films export without the media server running out of memory.
- **Analyze tab** — on-device analysis of a recorded ride (a phone-logger `SensPhone_*`/`GpsPhone_*` pair, or an Apple-Watch `WatchImu_*` motion stream). A per-second **mode strip** shows how each moment was spent — paddling / pumping vs gliding on the foil vs waiting — inferred from the IMU via an STFT cadence analysis, alongside **stroke cadence**, **board pitch / roll**, and **GPS speed** on one shared timeline. Every graph carries a **real time-of-day axis** (hh:mm:ss, from the recording's clock anchor) and its **y-axis scale value**, so a moment on the water can be matched to the clock and a peak to a number. **Pinch to zoom** into any stretch of a long session and drag to pan. Results are **cached on disk** — the first open of a long ride computes for a while, every open after that (including after an app restart) is instant. Everything is computed on the phone — nothing is uploaded.
- **GPS tab** — a **live readout** of the iPhone's built-in GNSS via `CoreLocation` (fix, rate, lat / lon / altitude / speed / accuracy) — an independent cross-check against the box GPS. To *record* a ride from the phone, use the **Phone logger** card below (GPS + motion); the old GPS-only CSV recorder was removed in v1.0.59 as a redundant subset of it.
- **Phone logger (box replacement)** — a card on the GPS tab records the iPhone's GPS **plus its motion sensors** (accelerometer, gyroscope, magnetometer, barometer at ~100 Hz) into the same Sens + Gps CSV pair the box writes — `SensPhone_*.csv` + `GpsPhone_*.csv`, with a phone-clock `# SYNC` anchor for drift-free Replay alignment. No box, no external receiver: mount the phone flat on the board (top edge toward the nose) and the full Replay pipeline works — pitch / Nasenwinkel, height above water, speed, and GPS track. The files are byte-compatible with the desktop and the other Movement Logger apps, so recordings are interchangeable. Recording keeps running with the screen locked (background location keeps the sensor stream alive); Share buttons export the finished pair.
- **Rides tab** — the ride list. Each row is one session's 1 Hz GPS track: Apple-Watch rides (synced from the watch app over WatchConnectivity) **and iPhone-recorded rides** from the Phone logger — so a ride logged with no Apple Watch connected still shows here (it falls back to a plain speed-gradient map, since there's no submersion column). Tap a ride to **plot it on a map**: an interactive Apple Maps view of the recorded track, plus a one-tap **shareable PNG** — real map tiles under an **activity-coloured track**, green start / red end markers, a legend, and a footer with the app logo, ride stats (top speed · distance · duration · median water temperature, on rides recorded by an Ultra · the **wind at the moment of top speed** from Apple's WeatherKit hourly history), and the GitHub source link. The same wind shows on the ride row, and the watch's live metrics add a **WIND** readout — the wind blowing when the session's TOP was set, relayed from the paired iPhone (shows "—" while the phone is out of reach). The raw CSV is still shareable straight from the row. (A standalone `scripts/ride_map_png.swift` renders the same PNG on macOS from any GPS CSV.)

  The activity comes from the Apple Watch Ultra's **submersion sensor**: wrist wet and slow → **in the water**, moving at riding pace → **on the board**, dry and slow away from the water → **on land**. Speed is only ever used to rule a mode *out* — nobody walks or swims at 16 km/h, so a moving fix is never "on land" — because at swim/foil speeds GPS noise crosses any threshold, and only submersion tells swimming from riding. Rides recorded before the submersion column existed can't tell water from land at all, so they degrade to a speed gradient (blue slow → red fast) rather than guessing. The map follows your phone's **light/dark appearance**, and the track colours flip with it (dark blue + dark green on the light map, light blue + crimson on the dark one) so they always read against the sea.
...
- **`1.x.x` = public App Store releases** — the only tags that auto-submit for review. The current store version is **1.0.55**.
...
A canonical backup of the slim Apple Distribution `.p12` and its password is kept outside this repo. Re-encrypt it with AES-256 when exporting — `security export` defaults to RC2-40, which OpenSSL 3 refuses to read without `-legacy`.
...
Push them to the current "Prepare for Submission" App Store version via the App Store Connect API (it reads the local Apple credentials file for the key path + issuer + key ID):
