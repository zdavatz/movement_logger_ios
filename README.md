...
- **GPS tab** — built-in iPhone GNSS via `CoreLocation` writes a `Gps`-schema CSV directly into `Documents/` so Replay can pick it up. **Share** + **Copy** buttons on the log card let you push the CSV out via the system share sheet or paste its content into another app.
