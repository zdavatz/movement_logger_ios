...
- **`1.x.x+` = public App Store releases** — the only tags that auto-submit for review. The live store version is **1.0.4**; the next store release is **1.0.5** (the App Store version string must be > the live one, and a build's `CFBundleShortVersionString` must match the store version it's submitted under, so a store build is literally built as `1.0.x`, not `0.0.x`).
