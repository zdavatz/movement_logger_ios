...
- **Rides** — the Apple-Watch ride list (`RidesScreen`). Each row is one watch session's 1 Hz GPS CSV (synced over WatchConnectivity into `Documents/WatchRides/` by `WatchRideReceiver`). Tapping a row opens **`RideMapView`** (v1.0.23+), which plots the recorded track on an interactive Apple `Map` and can export a **shareable PNG** — real map tiles under a speed-coloured track, start/end markers, and a branded footer (app logo + ride stats + the GitHub source link). See the *Rides tab — watch GPS on a map* section below. The raw CSV is still shareable straight from the row.
...
- `APPLE_WATCH_PROVISIONING_PROFILE_BASE64` — **(added v1.0.23, for the embedded Watch app)** base64 of the App Store distribution `.mobileprovision` for `ch.pumptsueri.movementlogger.watchkitapp`, bound to the **same** distribution cert. The build embeds the Apple Watch app, so its bundle id needs its OWN profile — the main-app profile can't sign it (Archive fails: *"app ID … does not match bundle ID …watchkitapp"* + missing HealthKit / Shallow-Depth entitlements). Generate with `scripts/create_watch_profile.py create` (reads the cert from the main-app CI profile so both match) and `gh secret set APPLE_WATCH_PROVISIONING_PROFILE_BASE64`. Re-roll whenever the distribution cert is re-rolled. **The Archive step deliberately passes NO `PROVISIONING_PROFILE_SPECIFIER`** — a global one forces both targets onto the main profile; with manual signing + only these two profiles installed on the clean runner, each target auto-selects its own by bundle id, and the Export step pins them per bundle id in `ExportOptions.plist`. (Store submission of a Watch-containing build ALSO needs an `APP_WATCH_SERIES_4` 368×448 screenshot — see the screenshots section / `scripts/upload_store_screenshots.py`.)
...
│   ├── RidesScreen.swift            Rides tab: watch-ride CSV list → RideMapView
│   └── RideMap.swift                RideMapView (interactive Map) + RideMapRenderer (MKMapSnapshotter → shareable PNG)
└── Export/
...
- **Shareable PNG** — the toolbar Share button calls `RideMapRenderer.render(rows:title:)`, which uses **`MKMapSnapshotter`** (NOT `ImageRenderer` — SwiftUI's `Map` snapshots blank because the tiles render out-of-process) to grab real Apple Maps tiles, then draws over the snapshot with CoreGraphics: a white-cased **speed-coloured** track (blue slow → red fast, `robustMaxSpeed` = 95th-pct so one GPS speed spike doesn't wash the scale), start/end dots, and a branded footer band carrying the **app logo** (`RideLogo` imageset — a copy of the app icon, since `UIImage(named:)` can't reliably load an `AppIcon` set), ride **stats** (top speed / distance via `GpsMath.haversineM` / duration), and the **GitHub source link** (`RideMapRenderer.sourceURL`). The PNG lands in `Documents/RideMaps/<name>_map.png` and is handed to a `UIActivityViewController` share sheet. `snapshot.point(for:)` returns points in the snapshot image's own space, so the track aligns to the tiles with no manual flip on iOS.
- **`scripts/ride_map_png.swift`** is the standalone macOS twin of `RideMapRenderer` (AppKit/`NSImage` instead of UIKit, and it DOES flip `point(for:)` because AppKit is y-up): `swift scripts/ride_map_png.swift <in.csv> <out.png> [logo.png]`. Used to generate a shareable ride PNG on the Mac without going through the phone.
