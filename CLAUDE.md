...
- **Rides** — the Apple-Watch ride list (`RidesScreen`). Each row is one watch session's 1 Hz GPS CSV (synced over WatchConnectivity into `Documents/WatchRides/` by `WatchRideReceiver`). Tapping a row opens **`RideMapView`** (v1.0.23+), which plots the recorded track on an interactive Apple `Map` as **one continuous line coloured by inferred activity** — in-water swim / on-board / on-land walk (v1.0.24+) — and can export a **shareable PNG** — real (dark) map tiles under the activity-coloured track, start/end markers, a legend, and a branded footer (app logo + ride stats + the GitHub source link). See the *Rides tab — watch GPS on a map* section below. The raw CSV is still shareable straight from the row.
...
**Both the interactive map and the shared PNG force DARK tiles** (the snapshot's
`traitCollection`, and `.environment(\.colorScheme, .dark)` scoped to the `Map` +
its legend). Apple's light map paints the sea a pale blue that a track has to
fight — the original green-on-water read as low contrast and blue "in water" was
near-invisible on the water it named. Dark navy sea + **cyan / crimson / amber**
gives every mode contrast against the water, the land, and each other, and the
tiles then match the dark footer.
